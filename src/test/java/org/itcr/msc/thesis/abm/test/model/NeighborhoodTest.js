...
class NeighborhoodEnv extends ABMEnv {
    ...
    step():void{
        ...
        switch (this.currStep){
            ...
            case 5:{
                this.patches().each((patch:NeighborhoodPatch)=>patch.alive=false)
                arrayToSet(this.patch.vonNewman(0,true)).each((patch:NeighborhoodPatch)=>patch.alive=true);
                break;
            }
            case 6:{
                this.patches().each((patch:NeighborhoodPatch)=>patch.alive=false)
                arrayToSet(this.patch.vonNewman(1)).each((patch:NeighborhoodPatch)=>patch.alive=true);
                break;
            }
            case 7:{
                this.patches().each((patch:NeighborhoodPatch)=>patch.alive=false)
                arrayToSet(this.patch.vonNewman(2)).each((patch:NeighborhoodPatch)=>patch.alive=true);
                break;
            }
            case 8:{
                this.patches().each((patch:NeighborhoodPatch)=>patch.alive=false)
                arrayToSet(this.patch.vonNewman(3)).each((patch:NeighborhoodPatch)=>patch.alive=true);
                break;
            }
            default: {
                this.complete = true;
